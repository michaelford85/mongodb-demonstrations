...
from lib.atlas_client import (REGION_ZONES, REGIONS, SHARD_KEYS, ZONE_BY_REGION,
                              db_name, get_db, is_collection_sharded, is_empty,
...
def render_multiregion() -> None:
    ...
    if not is_mongos():
        st.warning("Not connected through a mongos router. This page tells its "
                   ...
                   "`.env.example`; the topology below still applies.")
    ...
    st.markdown("#### Documents by region → owning zone (logical routing)")
    st.caption("Counts are routed by shard key on `region`, so they reflect the "
               "logical home zone of every document — the authoritative "
               "distribution. This is an Atlas Global Cluster, so Atlas manages "
               "physical chunk placement and raw $collStats per-shard counts can "
               "still show data on the primary shard; the region → zone view "
               "below is the stable, correct one.")
    rows = []
    for coll in SHARD_KEYS:
        rd = region_distribution(coll)
        rows.append({
            "collection": coll,
            "sharded": is_collection_sharded(coll),
            **{f"{r} ({ZONE_BY_REGION[r]})": rd.get(r, 0) for r in REGIONS},
        })
    st.dataframe(pd.DataFrame(rows).fillna(0), use_container_width=True,
                 hide_index=True)
