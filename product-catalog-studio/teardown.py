"""Tear down the Product Catalog Studio demo.

Drops this demo's Atlas Search and Vector Search indexes, drops the demo
database (all `pcs_*` collections), and frees the local Streamlit port. Only
touches this demo's database — no other database on the cluster is affected, and
no Atlas cluster is ever deleted.
...
from lib.atlas_client import COLLECTIONS, db_name, get_client, products
from lib.search import TEXT_INDEX, VECTOR_INDEX

SEARCH_INDEXES = [TEXT_INDEX, VECTOR_INDEX]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Tear down the Product Catalog Studio demo")
    ...
    name = db_name()
    print("This will:")
    print(f"  - drop search indexes {SEARCH_INDEXES}")
    if not args.keep_db:
