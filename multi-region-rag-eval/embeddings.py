...
Both backends share the same composed text for an account so that the
embedding spaces line up: client-side embeddings produced here for Postgres
are directly comparable to server-side embeddings generated by Atlas
Automated Embedding (which also runs against Voyage AI).
"""
